...
import { Notes, User } from '../../entities/dbdemo';
...
export class NotesService {
  ...
  async getAllNotes(userId: any) {
    // const data = await Notes.findAll({
    //   where: { userId }, // Filter notes by userId
    //   include: [{ model: User, attributes: ['userId', 'name', 'email'] }], // Include user information
    // });
    const data = await Notes.findAll({
      where: { userId },
      attributes: ['noteId', 'title', 'description', 'tag'],
    });
    return {
      status: 'success',
      data,
